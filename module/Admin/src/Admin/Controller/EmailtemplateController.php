...
class EmailtemplateController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_newsTable)){
            $this->_newsTable = $this->getServiceLocator()->get('Admin\Model\EmailTemplateTable');
        }
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Tin tức BĐS > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        
        $emailTemplateAdminForm       = $this->serviceLocator->get('FormElementManager')->get('emailTemplateAdminForm');
        

        if($this->getRequest()->isPost()){
            $data = $this->getRequest()->getPost();
            $emailTemplateAdminForm->setData($data);
            ...
            if(!empty($fileName)){
                ...
                if(!$upload->isValid('image')){
                    ...
                    //upload ảnh mới
                    $image = $upload->uploadFile('image', UPLOAD_PATH .'/email-template/', array('task' => 'rename'), 'batdongsan_');
                }
            } 

            if($emailTemplateAdminForm->isValid() && empty($error)){
                    ...
                    $this->redirect()->toUrl('/admin/emailtemplate/');
            }else{
                //echo '<pre>';
    ...
    public function editAction(){
        ...
        $emailTemplateAdminForm->setInputFilter(new \Admin\Form\EmailTemplateFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $emailTemplateAdminForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $emailTemplateAdminForm->setData($data);
