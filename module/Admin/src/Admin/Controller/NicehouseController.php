...
class NicehouseController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_niceHouseTable)){
            $this->_niceHouseTable = $this->getServiceLocator()->get('Admin\Model\NiceHouseTable');
        }
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Nhà đẹp > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        
        $niceHouseForm           = $this->serviceLocator->get('FormElementManager')->get('niceHouseForm');
        ...
        if($this->getRequest()->isPost()){
            ...
            if(!empty($fileName)){
                ...
                if(!$upload->isValid('image')){
                    ...
                    //upload ảnh mới
                    $image = $upload->uploadFile('image', UPLOAD_PATH .'/nice-house/', array('task' => 'rename'), 'batdongsan_');
                }
            } 

            if($niceHouseForm->isValid() && empty($error)){
                    ...
                    $this->redirect()->toUrl('/admin/nicehouse/');
            }else{
                //echo '<pre>';
    ...
    public function editAction(){
        ...
        $niceHouseForm->setInputFilter(new \Admin\Form\NiceHouseFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $niceHouseForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $niceHouseForm->setData($data);
    ...
    public function uploadAction(){
        ...
        if ($this->getRequest()->isXmlHttpRequest() == true){
            ...
            if($this->getRequest()->isPost()){
                ...
                if($upload->isValid('image')){
                    //upload ảnh mới
                    $fileName = $upload->uploadFile('image', UPLOAD_PATH .'/nice-house/', array('task' => 'rename'), 'batdongsan_');
                    //Thực hiện xóa ảnh cũ
                    
                    $upload->removeFile(UPLOAD_PATH .'/nice-house/'.$this->_arrPost['image_hidden']);
    ...
    public function deleteAction(){
        if(!empty($this->_arrParam['id'])){
            ...
            $upload = new \ZendVN\File\Upload();
            $upload->removeFile(UPLOAD_PATH .'/nice-house/'.$item->images);
