...
class NavFooterController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_modelTable)){
            $this->_modelTable = $this->getServiceLocator()->get('Admin\Model\NavFooterTable');
        }
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Quản lý menu (Footer) > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        $navFooterAdminForm       = $this->serviceLocator->get('FormElementManager')->get('navFooterAdminForm');
        
        $listItem          = $this->getTable()->itemInselectBox($this->_arrParam,array('task'=>'list-item'));
        
        if($this->getRequest()->isPost()){
            $data = $this->getRequest()->getPost();
            $navFooterAdminForm ->setData($data);
            //Kiểm tra chọn select box
            if(empty($this->_arrPost['parent'])){
                $error[]    = 'Bạn phải chọn danh mục! ';
            }

            if($navFooterAdminForm->isValid() && empty($error)){
                    ...
                    $this->redirect()->toUrl('/admin/nav-footer/');
            }
        }
    ...
    public function editAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Quản lý menu (Footer) > Chỉnh sửa';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        ...
        $navFooterAdminForm ->setInputFilter(new \Admin\Form\NavFooterFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $navFooterAdminForm ->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $navFooterAdminForm ->setData($data);
