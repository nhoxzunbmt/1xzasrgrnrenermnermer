...
class ServiceController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_modelTable)){
            $this->_modelTable = $this->getServiceLocator()->get('Admin\Model\ServiceTable');
        }
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Dịch vụ tài khoản cao cấp > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        
        $serviceAdminForm       = $this->serviceLocator->get('FormElementManager')->get('serviceAdminForm');
        
        
        if($this->getRequest()->isPost()){
            $data = $this->getRequest()->getPost();
            $serviceAdminForm->setData($data);


            if($serviceAdminForm->isValid() && empty($error)){
                    ...
                    $data    =  array(
                        ...
                        'normal'        => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['normal'])),
                        'vip'           => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['vip'])),
                        'hot'           => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['hot'])),
                        'free'          => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['free'])),
                        'chinhchu'      => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['chinhchu'])),
                    ...
                    $this->redirect()->toUrl('/admin/service/');
            }else{
                //echo '<pre>';
    ...
    public function editAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Dịch vụ tài khoản cao cấp > Chỉnh sửa';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        ...
        $serviceAdminForm->setInputFilter(new \Admin\Form\ServiceFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $serviceAdminForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $serviceAdminForm->setData($data);
