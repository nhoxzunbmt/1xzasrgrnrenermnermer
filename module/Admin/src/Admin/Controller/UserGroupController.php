...
class UserGroupController extends ActionController
{
    ...
    public function indexAction()
    {
        ...
        /*INSERT INTO `menu_test` (`id`,`name`,`status`,`parent`, `level` , `left` ,`right`) VALUES
        (1, 'Root', 1, 0, 0, 0, 23),
        (2, 'Chuyên ngành', 1, 1, 1, 1, 2),
        (3, 'Kinh tế', 1, 1, 1, 3, 12),
        (4, 'Tạp chí', 1, 1, 1, 13, 20),
        (5, 'Tiếng anh', 1, 1, 1, 21, 22),
        (6, 'Doanh nhân', 1, 3, 2, 4 5),
        (7, 'Khởi nghiệp', 1, 3, 2, 6, 7),
        (8, 'Lãnh đạo', 1, 3, 2, 8, 11),
        (9, 'IT', 1, 4, 2, 14, 17),
        (10, 'Sức khỏe', 1, 4, 2, 18, 19),
        (11, 'Nhân sự', 1, 8, 3, 9, 11),
        (12, 'E chip', 1, 9, 3, 15, 16);*/
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Quản lý nhóm khách hàng > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        
        $userGroupAdminForm       = $this->serviceLocator->get('FormElementManager')->get('userGroupAdminForm');
        
        
        if($this->getRequest()->isPost()){
            $data = $this->getRequest()->getPost();
            $userGroupAdminForm ->setData($data);


            if($userGroupAdminForm->isValid() && empty($error)){
               
                    
                    //Chống tấn công XSS
                    $purifier   = new \HTMLPurifier_HTMLPurifier();
                    $data    =  array(
                        'group_name'            => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['group_name'])),
                        'group_acp'             => $purifier->purify($this->_arrPost['group_acp']),
                        'group_default'         => $purifier->purify($this->_arrPost['group_default']),
                        'status'                => $purifier->purify($this->_arrPost['status']),
                        'color'                 => $purifier->purify($this->_arrPost['color']),
                    );
    ...
    public function editAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Loại hình doanh nghiệp > Chỉnh sửa';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        ...
        $userGroupAdminForm->setInputFilter(new \Admin\Form\UserGroupFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $userGroupAdminForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $userGroupAdminForm->setData($data);
            ...
            if($userGroupAdminForm->isValid() && empty($error)){
                ...
                //echo '<pre>';
                //print_r($userGroupAdminForm->getMessages());
                //echo '</pre>';
            }
        }
