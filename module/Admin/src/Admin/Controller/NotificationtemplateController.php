...
class NotificationtemplateController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_modelTable)){
            $this->_modelTable = $this->getServiceLocator()->get('Admin\Model\NotificationTemplateTable');
        }
    ...
    public function editAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Template thông báo > Chỉnh sửa';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        ...
        $notificationtemplateAdminForm       = $this->serviceLocator->get('FormElementManager')->get('notificationtemplateAdminForm');
        
        
        $notificationtemplateAdminForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $notificationtemplateAdminForm->setData($data);

            
            if($notificationtemplateAdminForm->isValid() && empty($error)){
                ...
                $this->redirect()->toUrl('/admin/notificationtemplate/');
            }else{
                //echo '<pre>';
        ...
        return new ViewModel(array(
            ...
            'arrParam'          =>  $this->_arrParam,
            'currentController' =>  $this->_currentController,
            'myForm'            =>  $notificationtemplateAdminForm,
            'item'              =>  $item,
            'error'             =>  $error,
        ));
    }
