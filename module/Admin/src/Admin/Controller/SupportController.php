...
class SupportController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_modelTable)){
            $this->_modelTable = $this->getServiceLocator()->get('Admin\Model\SupportTable');
        }
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Hỗ trợ trực tuyến > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        
        $supportAdminForm       = $this->serviceLocator->get('FormElementManager')->get('supportAdminForm');
        
        if($this->getRequest()->isPost()){
            $data = $this->getRequest()->getPost();
            $supportAdminForm->setData($data);


            if($supportAdminForm->isValid() && empty($error)){
               
                    //Chống tấn công XSS
                    $purifier   = new \HTMLPurifier_HTMLPurifier();
                    $data    =  array(
                        'name'          => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['name'])),
                        'nickname'      => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['nickname'])),
    ...
    public function editAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Hỗ trợ trực tuyến > Chỉnh sửa';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        ...
        $supportAdminForm->setInputFilter(new \Admin\Form\SupportFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $supportAdminForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $supportAdminForm->setData($data);
