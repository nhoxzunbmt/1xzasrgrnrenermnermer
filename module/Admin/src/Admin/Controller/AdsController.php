...
class AdsController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_newsTable)){
            $this->_newsTable = $this->getServiceLocator()->get('Admin\Model\AdsTable');
        }
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Quảng cáo trực tuyến > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        //date_default_timezone_set("America/Los_Angeles");
        //echo date('d/m/y h:i:s');
        $adsAdminForm       = $this->serviceLocator->get('FormElementManager')->get('adsAdminForm');
        

        if($this->getRequest()->isPost()){
            $data = $this->getRequest()->getPost();
            $adsAdminForm->setData($data);
            ...
            if(!empty($fileName)){
                ...
                if(!$upload->isValid('image')){
                    ...
                    //upload ảnh mới
                    $flash_image = $upload->uploadFile('image', UPLOAD_PATH .'/ads/', array('task' => 'rename'), 'batdongsan_');
                }
            }


            //Kiểm tra chọn select box
            if(empty($this->_arrPost['area_ads'])){
                $error[]    = 'Bạn phải chọn Khu vực quảng cáo!';
            }

            if($adsAdminForm->isValid() && empty($error)){
                    ...
                    $data    =  array(
                        ...
                        'area_ads'      => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['area_ads'])),
    ...
    public function editAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Quảng cáo trực tuyến > Chỉnh sửa';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        ...
        $adsAdminForm->setInputFilter(new \Admin\Form\AdsFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $adsAdminForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $adsAdminForm->setData($data);
    ...
    public function uploadAction(){
        ...
        if ($this->getRequest()->isXmlHttpRequest() == true){
            ...
            if($this->getRequest()->isPost()){
                ...
                if($upload->isValid('image')){
                    //upload ảnh mới
                    $fileName = $upload->uploadFile('image', UPLOAD_PATH .'/ads/', array('task' => 'rename'), 'batdongsan_');
                    //Thực hiện xóa ảnh cũ
                    
                    $upload->removeFile(UPLOAD_PATH .'/ads/'.$this->_arrPost['image_hidden']);
    ...
    public function deleteAction(){
        if(!empty($this->_arrParam['id'])){
            ...
            $upload = new \ZendVN\File\Upload();
            $upload->removeFile(UPLOAD_PATH .'/ads/'.$item->images);
