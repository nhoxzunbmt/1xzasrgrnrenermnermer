...
class TvController extends AbstractActionController
{
    ...
    public function getTable(){
        if(empty($this->_tvTable)){
            $this->_tvTable = $this->getServiceLocator()->get('Admin\Model\TvTable');
        }
        return $this->_tvTable;
    }
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Bất động sản TV > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        
        $tvForm       = $this->serviceLocator->get('FormElementManager')->get('tvForm');
        ...
        if($this->getRequest()->isPost()){
            ...
            if(!empty($fileName)){
                ...
                if(!$upload->isValid('image')){
                    ...
                    //upload ảnh mới
                    $image = $upload->uploadFile('image', UPLOAD_PATH .'/tv/', array('task' => 'rename'), 'batdongsan_');
                }
            } 

            if($tvForm->isValid() && empty($error)){
                    ...
                    $data    =  array(
                        ...
                        'link_youtube'  => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['link_youtube'])), 
                        'images'        => $image,
                        'date_time'     => date('d/m/y h:i:s'),
                    );
                    $this->getTable()->saveItem($data,array('task'=>'add'));
                    $this->flashMessenger()->addSuccessMessage('Dữ liệu đã được lưu thành công');
                    $this->redirect()->toUrl('/admin/tv/');
            }else{
                //echo '<pre>';
    ...
    public function editAction(){
        ...
        $newsForm       = $this->serviceLocator->get('FormElementManager')->get('newsForm');
        
        $newsForm->setInputFilter(new \Admin\Form\NewsFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $newsForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            ...
            if($newsForm->isValid() && empty($error)){
                ...
                $this->flashMessenger()->addSuccessMessage('Dữ liệu đã được lưu thành công');
                $this->redirect()->toUrl('/admin/news/');
            }else{
                //echo '<pre>';
                //print_r($newsForm->getMessages());
                //echo '</pre>';
    ...
    public function uploadAction(){
        ...
        if ($this->getRequest()->isXmlHttpRequest() == true){
            ...
            if($this->getRequest()->isPost()){
                ...
                if($upload->isValid('image')){
                    //upload ảnh mới
                    $fileName = $upload->uploadFile('image', UPLOAD_PATH .'/news/', array('task' => 'rename'), 'batdongsan_');
                    //Thực hiện xóa ảnh cũ
                    
                    $upload->removeFile(UPLOAD_PATH .'/news/'.$this->_arrPost['image_hidden']);
    ...
    public function filterAction(){
        ...
        if($this->_arrParam['type'] == 'search'){
            if($this->_arrParam['col'] == 'null' && $this->_arrParam['by'] == 'null' && $this->_arrParam['key'] == 1){
                if(!empty($this->params()->fromPost('keywords'))){
