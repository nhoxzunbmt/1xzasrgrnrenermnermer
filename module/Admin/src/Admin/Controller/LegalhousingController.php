...
class LegalhousingController extends ActionController
{
    ...
    protected $_legalHousingTable;


    public function getTable(){
        if(empty($this->_legalHousingTable)){
            $this->_legalHousingTable = $this->getServiceLocator()->get('Admin\Model\LegalHousingTable');
        }
        return $this->_legalHousingTable;
    }
    ...
    public function indexAction()
    {
        ...
        $itemsGroup         = $this->getTable()->itemInselectBox($this->_arrParam,array('task'=>'list-item-category-legal-housing'));
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Pháp lý nhà đất > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        
        $legalhousingForm           = $this->serviceLocator->get('FormElementManager')->get('legalhousingForm');
       
        //Danh mục
        $itemsCategoryLegalHousing  =   $this->getTable()->itemInselectBox($this->_arrParam,array('task'=>'list-item-category-legal-housing'));
        
        if($this->getRequest()->isPost()){
            $data = $this->getRequest()->getPost();
            $legalhousingForm->setData($data);
            ...
            if(!empty($fileName)){
                ...
                if(!$upload->isValid('image')){
                    ...
                    //upload ảnh mới
                    $image = $upload->uploadFile('image', UPLOAD_PATH .'/legal-housing/', array('task' => 'rename'), 'batdongsan_');
                }
            } 

            if($legalhousingForm->isValid() && empty($error)){
                    ...
                    $this->redirect()->toUrl('/admin/legalhousing/');
            }else{
                //echo '<pre>';
        ...
        return new ViewModel(array(
            ...
            'arrParam'          =>  $this->_arrParam,
            'currentController' =>  $this->_currentController,
            'myForm'            =>  $legalhousingForm,
            'itemsCategoryLegalHousing'=>  $itemsCategoryLegalHousing,
            'error'             =>  $error,
        ));
    }
    public function editAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Pháp lý nhà đất > Chỉnh sửa';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
       
        //Danh mục
        $itemsCategoryNews  =   $this->getTable()->itemInselectBox($this->_arrParam,array('task'=>'list-item-category-legal-housing'));
        ...
        $legalhousingForm->setInputFilter(new \Admin\Form\LegalHousingFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $legalhousingForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $legalhousingForm->setData($data);
    ...
    public function uploadAction(){
        ...
        if ($this->getRequest()->isXmlHttpRequest() == true){
            ...
            if($this->getRequest()->isPost()){
                ...
                if($upload->isValid('image')){
                    //upload ảnh mới
                    $fileName = $upload->uploadFile('image', UPLOAD_PATH .'/legal-housing/', array('task' => 'rename'), 'batdongsan_');
                    //Thực hiện xóa ảnh cũ
                    
                    $upload->removeFile(UPLOAD_PATH .'/legal-housing/'.$this->_arrPost['image_hidden']);
    ...
    public function deleteAction(){
        if(!empty($this->_arrParam['id'])){
            ...
            $upload = new \ZendVN\File\Upload();
            $upload->removeFile(UPLOAD_PATH .'/legal-housing/'.$item->images);
