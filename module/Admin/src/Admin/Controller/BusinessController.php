...
class BusinessController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_businessTable)){
            $this->_businessTable = $this->getServiceLocator()->get('Admin\Model\BusinessTable');
        }
    ...
    {
    	//Tiêu đề
    	$title      =  'Danh sách doanh nghiệp BĐS';   
        ...
        $itemsGroup         = $this->getTable()->itemInselectBox($this->_arrParam,array('task'=>'list-item-type-business'));
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Doanh nghiệp BĐS > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        
        $businessForm       = $this->serviceLocator->get('FormElementManager')->get('businessForm');
        ...
        if($this->getRequest()->isPost()){
            ...
            if($businessForm->isValid() && empty($error)){
                    ...
                    $this->redirect()->toUrl('/admin/business/');
            }else{
                //echo '<pre>';
    ...
    public function editAction(){
        ...
        $businessForm       = $this->serviceLocator->get('FormElementManager')->get('businessForm');
        $businessForm->setInputFilter(new \Admin\Form\BusinessFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
    ...
    public function deleteAction(){
        if(!empty($this->_arrParam['id'])){
            ...
            if($item->group_id != 1){
                ...
                $upload = new \ZendVN\File\Upload();
                $upload->removeFile(UPLOAD_PATH .'/logo-business/'.$item->logo);
