...
class ContractformController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_contractFormTable)){
            $this->_contractFormTable = $this->getServiceLocator()->get('Admin\Model\ContractFormTable');
        }
    ...
    public function listAction(){
        ...
        if ($this->getRequest()->isXmlHttpRequest() == true){
            ...
            $totalItem      = $this->getTable()->countItem($this->_arrParam,array('task'=>'count-item'));
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Biểu mẫu hợp đồng > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        
        $contractFormForm       = $this->serviceLocator->get('FormElementManager')->get('contractFormForm');
        
        //Danh mục
        $itemsCategoryNews  =   $this->getTable()->itemInselectBox($this->_arrParam,array('task'=>'list-item-category'));
        
        if($this->getRequest()->isPost()){
            $data = $this->getRequest()->getPost();
            $contractFormForm ->setData($data);
            ...
            if(!empty($fileName)){
                ...
                if(!$upload->isValid('file')){
                    ...
                    //upload ảnh mới
                    $file = $upload->uploadFile('file', UPLOAD_PATH .'/contract-form/', array('task' => 'rename'), 'batdongsan_');
                }
            } 

            if($contractFormForm->isValid() && empty($error)){
                    ...
                    $this->redirect()->toUrl('/admin/contractform/');
            }else{
                //echo '<pre>';
    ...
    public function editAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Biểu mẫu hợp đồng > Chỉnh sửa';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        ...
        $contractFormForm->setInputFilter(new \Admin\Form\ContractFormFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $contractFormForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $contractFormForm->setData($data);
    ...
    public function uploadAction(){
        ...
        if ($this->getRequest()->isXmlHttpRequest() == true){
            ...
            if($this->getRequest()->isPost()){
                ...
                if($upload->isValid('file')){
                    //upload ảnh mới
                    $fileName = $upload->uploadFile('file', UPLOAD_PATH .'/contract-form/', array('task' => 'rename'), 'batdongsan_');
                    //Thực hiện xóa ảnh cũ
                    
                    $upload->removeFile(UPLOAD_PATH .'/contract-form/'.$this->_arrPost['image_hidden']);
    ...
    public function deleteAction(){
        if(!empty($this->_arrParam['id'])){
            ...
            $upload = new \ZendVN\File\Upload();
            $upload->removeFile(UPLOAD_PATH .'/contract-form/'.$item->file);  
