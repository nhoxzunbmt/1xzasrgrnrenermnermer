...
class PermissionController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_userGroupTable)){
            $this->_userGroupTable = $this->getServiceLocator()->get('Admin\Model\PermissionTable');
        }
    ...
    public function editAjaxAction(){
        ...
        if ($this->getRequest()->isXmlHttpRequest() == true){
            ...
            if($this->params()->fromQuery() == true){
                ...
                $arrPrivilege = \Zend\Json\Json::decode($item['permission_id']);
                
                if(!empty($option) && $option == 'add'){
                    //Set permission
                    ...
                    $privilege_id[]     = (int) $this->params()->fromQuery('privilege_id');
                    $arrParam = array(
                        'id'            =>  $this->params()->fromQuery('id'),
                        'permission_id' =>  \Zend\Json\Json::encode($privilege_id),
                    );
                    $this->getTable()->saveItem($arrParam,array('task'=>'setPermiss'));
                }else if(!empty($option) && $option == 'remove'){
                    //Remove permission
                    if(!empty($arrPrivilege)){
                        
                        //Chuyển value thành key
                        $newArray = array_flip($arrPrivilege);
                        
                        $privilege_id = (int) $this->params()->fromQuery('privilege_id');
                        unset($newArray[$privilege_id]);
                        
                        $newArrPrivilege = array_flip($newArray);
                    }
                    $arrParam = array(
                        'id'            =>  $this->params()->fromQuery('id'),
                        'permission_id' =>  \Zend\Json\Json::encode($newArrPrivilege),
                    );
                    $this->getTable()->saveItem($arrParam,array('task'=>'setPermiss'));
                }
            }
    ...
    public function previewAction(){
        $view = new ViewModel();
        $parents = $this->getTable()->listItem($this->_arrParam,array('task'=>'list-permission'));
        $item = $this->getTable()->getItem($this->_arrParam);
