...
class SitesController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_modelTable)){
            $this->_modelTable = $this->getServiceLocator()->get('Admin\Model\SitesTable');
        }
    ...
    public function listDistrictAction()
    {
        ...
        //Tiêu đề
        $title      =  'Các quận, huyện của '. $item->name;   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
     ...
     //Ajax load indexAction
    public function listDistrictAjaxAction(){
        $view               = new ViewModel();
        $dataItems          = null;
        $dataPaginator      = null;
        $isXmlHttpRequest   = false;
        ...
        if ($this->getRequest()->isXmlHttpRequest() == true){
            ...
            $totalItem      = $this->getTable()->countItem($this->_arrParam,array('task'=>'district'));
            $items          = $this->getTable()->listItem($this->_arrParam,array('task'=>'list-items-paginator-district'));
    ...
    public function listWardAction()
    {
        ...
        $item           = $this->getTable()->getItem($this->_arrParam,array('task'=>'district'));
        
        $itemCity       = $this->getTable()->getItem(array('id'=>$item['city_id']),array('task'=>'get-item'));
        //Tiêu đề
        $title      =  'Các xã, phường của '. $item['name'] .' - '.$itemCity->name;   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        $redirect   = new Container($this->_namespace.'_redirect');
        $redirect->curentUrl = '/admin/sites/list-ward/'.$item['id'];
        return new ViewModel(array(
            'title'             =>  $title,
            'arrParam'          =>  $this->_arrParam,
     ...
     //Ajax load indexAction
    public function listWardAjaxAction(){
        $view               = new ViewModel();
        $dataItems          = null;
        $dataPaginator      = null;
        $isXmlHttpRequest   = false;
        ...
        if ($this->getRequest()->isXmlHttpRequest() == true){
            ...
            $this->_arrParam['idWard'] = $this->params()->fromQuery('idWard');
            $totalItem      = $this->getTable()->countItem($this->_arrParam,array('task'=>'ward'));
            $items          = $this->getTable()->listItem($this->_arrParam,array('task'=>'list-items-paginator-ward'));
    ...
    public function editAction(){
        ...
        $sitesAdminForm       = $this->serviceLocator->get('FormElementManager')->get('sitesAdminForm');
        
        $sitesAdminForm->setInputFilter(new \Admin\Form\SitesFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $sitesAdminForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $sitesAdminForm->setData($data);

            
            if($sitesAdminForm->isValid() && empty($error)){
               
                //Chống tấn công XSS
                $purifier   = new \HTMLPurifier_HTMLPurifier();
                $data    =  array(
                    'id'            => $purifier->purify($this->_arrPost['id']), 
                    'name'        => $purifier->purify($this->_arrPost['name']),
                );

                $this->getTable()->saveItem($data,array('task'=>'edit'));
                $this->flashMessenger()->addSuccessMessage('Dữ liệu đã được lưu thành công');
                $this->redirect()->toUrl('/admin/sites/');
            }else{
                //echo '<pre>';
    ...
    public function editDistrictAction(){
        ...
        if($this->getRequest()->isPost()){
            ...
            if($sitesAdminForm->isValid() && empty($error)){
                ...
                $purifier   = new \HTMLPurifier_HTMLPurifier();
                $data    =  array(
                    'id'            => $purifier->purify($item['id']), 
                    'name'        => $purifier->purify($this->_arrPost['name']),
                );



                $this->getTable()->saveItem($data,array('task'=>'edit-district'));
                $this->flashMessenger()->addSuccessMessage('Dữ liệu đã được lưu thành công');
                $this->redirect()->toUrl('/admin/sites/list-district/'.$item['city_id']);
            }else{
                //echo '<pre>';
                //print_r($newsForm->getMessages());
                //echo '</pre>';
    ...
    public function editWardAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Xã phường > Chỉnh sửa';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
    
        $item           = $this->getTable()->getItem($this->_arrParam,array('task'=>'ward'));
        ...
        if($this->getRequest()->isPost()){
            ...
            if($sitesAdminForm->isValid() && empty($error)){
                ...
                $this->getTable()->saveItem($data,array('task'=>'edit-ward'));
                $this->flashMessenger()->addSuccessMessage('Dữ liệu đã được lưu thành công');
                $this->redirect()->toUrl('/admin/sites/list-ward/'.$item['district_id']);
            }else{
                //echo '<pre>';
                //print_r($newsForm->getMessages());
                //echo '</pre>';
    ...
    public function addDistrictAction(){
        ...
        if($this->getRequest()->isPost()){
            ...
            if($sitesAdminForm->isValid() && empty($error)){
                    ...
                    $this->getTable()->saveItem($data,array('task'=>'add-district'));
                    $this->flashMessenger()->addSuccessMessage('Dữ liệu đã được lưu thành công');
                    $this->redirect()->toUrl('/admin/sites/list-district/'.$this->_arrParam['id']);
            }else{
                //echo '<pre>';
                //print_r($newsForm->getMessages());
                //echo '</pre>';
    ...
    public function addWardAction(){
        ...
        if($this->getRequest()->isPost()){
            ...
            if($sitesAdminForm->isValid() && empty($error)){
                    ...
                    $this->getTable()->saveItem($data,array('task'=>'add-ward'));
                    $this->flashMessenger()->addSuccessMessage('Dữ liệu đã được lưu thành công');
                    $this->redirect()->toUrl('/admin/sites/list-ward/'.$this->_arrParam['id']);
            }else{
                //echo '<pre>';
                //print_r($newsForm->getMessages());
                //echo '</pre>';
    ...
    public function deleteWardAction(){
        if(!empty($this->_arrParam['id'])){
            $filter   = new \Zend\Filter\Digits();
            $id       = $filter->filter($this->_arrParam['id']);
            $arrParam = array('id'=>$id);
            ...
            $this->getTable()->deleteItem($arrParam,array('task'=>'delete-item-ward'));
    ...
    public function deleteDistrictAction(){
        if(!empty($this->_arrParam['id'])){
            ...
            $this->getTable()->deleteItem($arrParam,array('task'=>'delete-item-district'));
