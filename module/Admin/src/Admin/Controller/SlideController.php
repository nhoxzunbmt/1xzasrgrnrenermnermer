...
class SlideController extends ActionController
{
    ...
    public function getTable(){
        if(empty($this->_newsTable)){
            $this->_newsTable = $this->getServiceLocator()->get('Admin\Model\SlideTable');
        }
    ...
    public function addAction(){
        $error          = array();
        //Tiêu đề
        $title          =  'Slide show > Thêm mới';   
        $this->headTitle($title)->setSeparator(" - ")->append("Hệ thống quản trị website");
        
        //date_default_timezone_set("America/Los_Angeles");
        //echo date('d/m/y h:i:s');
        $slideAdminForm       = $this->serviceLocator->get('FormElementManager')->get('slideAdminForm');
        

        if($this->getRequest()->isPost()){
            $data = $this->getRequest()->getPost();
            $slideAdminForm->setData($data);
            ...
            if(!empty($fileName)){
                ...
                if(!$upload->isValid('image')){
                    ...
                    //upload ảnh mới
                    $images = $upload->uploadFile('image', UPLOAD_PATH .'/slide/', array('task' => 'rename'), 'batdongsan_');
                }
            }



            if($slideAdminForm->isValid() && empty($error)){
                    ...
                    $data    =  array(
                        ...
                        'images'         => $images,
                        'link'           => $purifier->purify($this->_viewHelper->cmsReplaceString($this->_arrPost['link'])),
                        'open'           => $this->_arrPost['open'],
                        'date_time'      => date('d/m/y h:i:s'),
                    );
                    $this->getTable()->saveItem($data,array('task'=>'add'));
                    $this->flashMessenger()->addSuccessMessage('Dữ liệu đã được lưu thành công');
                    $this->redirect()->toUrl('/admin/slide/');
            }else{
                //echo '<pre>';
    ...
    public function editAction(){
        ...
        $slideAdminForm->setInputFilter(new \Admin\Form\AdsFilter(array('task'=>'edit', 'id'=>$this->_arrParam['id'])));
        $slideAdminForm->bind($item);
        if($this->getRequest()->isPost()){
            $data   = $this->getRequest()->getPost();
            $slideAdminForm->setData($data);
    ...
    public function uploadAction(){
        ...
        if ($this->getRequest()->isXmlHttpRequest() == true){
            ...
            if($this->getRequest()->isPost()){
                ...
                if($upload->isValid('image')){
                    //upload ảnh mới
                    $fileName = $upload->uploadFile('image', UPLOAD_PATH .'/slide/', array('task' => 'rename'), 'batdongsan_');
                    //Thực hiện xóa ảnh cũ
                    
                    $upload->removeFile(UPLOAD_PATH .'/slide/'.$this->_arrPost['image_hidden']);
    ...
    public function deleteAction(){
        if(!empty($this->_arrParam['id'])){
            ...
            $upload = new \ZendVN\File\Upload();
            $upload->removeFile(UPLOAD_PATH .'/slide/'.$item->images);
