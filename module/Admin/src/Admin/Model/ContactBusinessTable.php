...
class ContactBusinessTable{
   ...
   public function listItem($arrParam = null,$options = null){
      ...
      if($options['task'] == 'list-items-paginator'){
         ...
         $sqlObj     = new Sql($this->adapter);
         $select     = $sqlObj->select();
         $select->from(array('c' => 'contact_business'))
   ...
   public function getItem($arrParam = null, $options = null){
      ...
      if($options == null){
         ...
         $select     = $sqlObj->select()
                              ...
                              $select->join(
                                 array('u' => 'users'),
                                 'u.id = contact_business.user_id',
                                 array('fullname_contact' => 'fullname'),
                                 $select::JOIN_LEFT
                              );
                              $select->where('contact_business.id ='.$arrParam['id']);
         $result     = \ZendVN\Db\Sql\Result::toArray($select,$this->adapter);
         $result = $result[0];
         return $result;
      }
