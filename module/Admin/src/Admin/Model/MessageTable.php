...
class MessageTable{
   ...
   public function countItem($arrParam = null,$options = null){
      ...
      if($options['task'] == 'count-hop-thu-den-user-message'){
         ...
         $select     = $sqlObj->select()
                              ...
                              /*if(!empty($ssFilter['keywords_user_receive']) && !empty($ssFilter['field_user_receive'])){
                                 $keywords = '%' . $ssFilter['keywords_user_receive'] . '%';
                                 $select->where->like($ssFilter['field_user_receive'], $keywords);
                              }*/
      ...
      if($options['task'] == 'count-hop-thu-di-user-message'){
         $ssFilter   = $arrParam['ssFilter'];
         $sqlObj     = new Sql($this->adapter);
         $select     = $sqlObj->select()
                              ->from(array('m' => 'message_send'))
                           ...
                           /*if(!empty($ssFilter['keywords_user_send']) && !empty($ssFilter['field_user_send'])){
                              $keywords = '%' . $ssFilter['keywords_user_send'] . '%';
                              $select->where->like($ssFilter['field_user_send'], $keywords);
                           } */
   ...
   public function listItem($arrParam = null,$options = null){
      ...
      if($options['task'] == 'list-item-hop-thu-di'){
         ...
         $select->from(array('m' => 'message_send'))
               ...
               if(!empty($ssFilter['keywords_user_send']) && !empty($ssFilter['field_send'])){
                  $keywords = '%' . $ssFilter['keywords_send'] . '%';
                  $select->where->like($ssFilter['field_send'], $keywords);
               }
      ...
      if($options['task'] == 'list-item-hop-thu-di-user-message'){
         $paginator  = $arrParam['paginator'];
         $ssFilter   = $arrParam['ssFilter'];
         ...
         $select->from(array('m' => 'message_send'))
               ...
               ->join(
                  array('u' => 'users'),
                  'u.id = m.user_id_receive',
                  array('fullname_receive'=>'fullname'),
                  $select::JOIN_LEFT
               )

               ->join(
                  array('u1' => 'users'),
                  'u1.id = m.user_id_send',
                  array('fullname_send'=>'fullname'),
                  $select::JOIN_LEFT
               )
      ...
      if($options['task'] == 'list-item-hop-thu-den'){
         ...
         $select->from(array('m' => 'message_receive'))
               ...
               $select->where('m.user_id_receive ='.$arrParam['idUser']); 
                //-------Lọc theo keyword và field----
               if(!empty($ssFilter['keywords_receive']) && !empty($ssFilter['field_receive'])){
                  $keywords = '%' . $ssFilter['keywords_receive'] . '%';
                  $select->where->like($ssFilter['field_receive'], $keywords);
               }
       ...
       if($options['task'] == 'list-item-hop-thu-den-user-message'){
         $paginator  = $arrParam['paginator'];
         $ssFilter   = $arrParam['ssFilter'];
   ...
   public function getItem($arrParam = null, $options = null){
      ...
      if($options['task'] == 'view-receive-user-message'){
         ...
         $select     = $sqlObj->select()
                              ...
                              $select->join(
                                 array('u1' => 'users'),
                                 'u1.id = message_receive.user_id_receive',
                                 array('fullname_receive'=>'fullname'),
                                 $select::JOIN_LEFT
                              );
                              $select->where('message_receive.id ='.$arrParam['id']);
