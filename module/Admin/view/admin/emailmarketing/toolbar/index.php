<?php 
	...
	$linkAddNew 		= $this->basePath($this->currentController . '/change-template');
	...
	}else if($this->arrParam['action'] == 'change-template'){
		$label 			= 'Chọn';
		$linkSaveItem 	= $this->basePath($this->currentController . '/change-template');
	}else {
		$label 			= 'Lưu';
		$linkSaveItem 	= $this->basePath($this->currentController . '/edit/' . $id);
	}
	$btnSaveItem 		= $this->cmsButton($label,$linkSaveItem,TEMPLATE_URL . '/admin/images/icon-32-save.png','submit');
	...
	switch ($this->arrParam['action']){
		...
		case 'change-template' 		: 	$strBtn = $btnSaveItem . ' ' . $btnCancel;
							$iconTitle 	= 'document_add.png';
			break;		
