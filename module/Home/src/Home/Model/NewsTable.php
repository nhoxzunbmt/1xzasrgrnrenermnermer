...
class NewsTable{
      ...
      if($options == null){
         ...
         $select     = $sqlObj->select()
                              ...
                              if(!empty($arrParam['cat_id'])){
                                 $select->where('n.cat_id = '.$arrParam['cat_id']);
                              }
                              if(!empty($arrParam['cityid'])){
                                 $select->where('n.city = '.$arrParam['cityid']);
                              } 
   ...
   public function listItem($arrParam = null,$options = null){
      ...
      if($options['task'] == 'block-website-ads'){
         
         $sqlObj     = new Sql($this->adapter);
         $select     = $sqlObj->select()
                              ->from(array('w'=>'website_ads'))
                              ->columns(array(
                                 'id','name','url'
                              ));
   ...
   public function saveItem($arrParam = null,$options = null){
      ...
      if($options['task'] == 'email-newsLetter'){
         $sqlObj     = new Sql($this->adapter);
         $insertObj  = $sqlObj->insert('email_newsletter');
